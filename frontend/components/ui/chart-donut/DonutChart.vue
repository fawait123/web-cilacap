...
<template>
    <div :class="cn('grid grid-cols-2', $attrs.class ?? '')">
        <VisSingleContainer :style="{ height: isMounted ? '100%' : 'auto' }" :margin="{ left: 20, right: 20 }"
            :data="data">
        ...
        <div class="flex flex-col gap-5">
            <div v-for="(color, i) in props.colors" class="mb-2 flex gap-4 items-center">
                <div class="w-4 h-4 rounded-full" :style="{ background: color }" />
                <span class="text-[12px] font-semibold">{{ data[i][props.index] }} <span class="text-primary">{{
                    data[i][props.category] }}</span></span>
            </div>
        </div>
    </div>
</template>
